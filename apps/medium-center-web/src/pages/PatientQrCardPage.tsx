...
export function PatientQrCardPage() {
  ...
  async function regenerateQr() {
    if (!card || !window.confirm("سيتم إلغاء الرمز السابق وإصدار رمز جديد للبطاقة. هل تريد المتابعة؟")) {
      return;
    }
    ...
    try {
      ...
      setError("");
      setCard(
        await apiRequest<PatientQrCard>(`/center/patients/${card.patient.id}/regenerate-qr`, {
          method: "POST"
        })
      );
    } catch (cause) {
  ...
  return (
    <div className="page-stack qr-page">
        ...
        <div className="button-row">
          ...
          {canRegenerate ? (
            <button className="danger-button" disabled={regenerating} type="button" onClick={() => void regenerateQr()}>
              {regenerating ? "جار التجديد..." : "تجديد رمز QR"}
            </button>
          ) : null}
        </div>
      </div>
      ...
      {loading ? <div className="loading-state">جار تحميل بطاقة المريض...</div> : null}
      ...
      {!loading && card ? (
        <section className="patient-qr-card" aria-label="بطاقة المريض الرقمية">
          ...
          <aside className="qr-card-code">
            ...
            <span className="qr-token">{card.qrToken}</span>
            <small>يحتوي الرمز على معرف آمن فقط ولا يحتوي على بيانات طبية.</small>
          </aside>
        </section>
      ) : null}
      ...
      <div className="no-print">
        <SectionCard
          title="التحقق من بطاقة QR"
          subtitle="يمكن للموظف إدخال الرمز أو قيمة QR كما هي لفتح بطاقة المريض داخل نفس المركز."
        >
