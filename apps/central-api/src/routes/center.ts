...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!patient) {
      throw new AppError("تعذر العثور على بطاقة المريض داخل هذا المركز.", 404);
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!patient) {
      throw new AppError("رمز بطاقة المريض غير صالح أو لا يتبع لهذا المركز.", 404);
    }
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!alert) {
      throw new AppError("الإشعار غير موجود داخل هذا المركز.", 404);
    }
